...
if(isset($_POST['submit']))
{
	...
	else
	{
		...
		$firstname = db_getFirstResult("select firstname from s_users where username='$username'");
		if(empty($firstname))
			$firstname = "SWE Database User";
		// Create new confirmation has
		$hash = random_hash();
		$sql = "update s_users set confirm_hash='$hash' where username='$username' limit 1";
		db_query($sql);
...
if(isset($_POST['reset_password']))
{
	...
	else if(strlen($pwd1)!= 0 && strlen($username)!=0 && $validate==true)
	{
		...
		db_update("s_users",$sql_arr,"username='$username'");
...
if(isset($_GET['confirm']) && isset($_GET['username']))
{
	...
	$sql = "select username from s_users where username='$username' and confirm_hash='$hash'";
	...
	else
	{
		...
		<?php
		echo "<table><tr>";
		echo "<td>E-mail:</td><td><input type='text' disabled value='$username@mit.edu'><input type='hidden' name='username' value='$username'></td></tr>";
...
<script type = "text/javascript" src = "<?php echo $prepath;?>/ajaxcore/prototype.js"></script>
<script type = "text/javascript" src = "<?php echo $prepath;?>/ajaxcore/AjaxCore.js"></script>
<script type = "text/javascript" src = "<?php echo $prepath;?>/includes/validate.js"></script>
